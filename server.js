    ...
    anthropicRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: [
          ...
          { type: 'text', text: `You are a UK window and door expert. Detect every window and door visible on the exterior of this building. Return ONLY a JSON array, no markdown, no explanation.\n\nType guide — read carefully before classifying:\n- window-sash: slides UP/DOWN (two overlapping panels, horizontal meeting rail across middle, common on Victorian/Edwardian terraces). THIS IS NOT a casement.\n- window-casement: hinges on the SIDE and swings outward. Has visible hinges on one edge.\n- window-flush: modern flat/flush frame, no projecting sash, very clean lines.\n- window-double: two side-by-side casement panes in one frame.\n- window-bay: projects outward from the wall at an angle (3 panels forming a bay).\n- window-tilt: tilt-and-turn, usually large single pane, commercial/modern style.\n- window-skylight: in the ROOF, overhead.\n- door-single: one solid panel door.\n- door-double: two equal solid panels.\n- door-bifold: multiple narrow folding panels.\n- door-sliding: panels slide horizontally.\n- door-french: two glazed panels opening outward.\n- door-patio: large sliding glazed door.\n\nIf a property has tall narrow windows with a horizontal bar across the middle — classify as window-sash, not window-casement.\n\nEach item: {"type":"<type>","confidence":0.0-1.0,"x_pct":0-100,"y_pct":0-100,"w_pct":1-100,"h_pct":1-100}. Add a final item {"type":"analysis","summary":"2-3 sentence overview including predominant window type and style period"}.` }
        ]}]
      })
    });
  } catch (err) {
